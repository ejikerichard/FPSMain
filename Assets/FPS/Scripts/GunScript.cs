    ...
    if (waitTillNextFire <= 0 && !reloading && pmS.maxSpeed < 5)
    {
        if (bulletsInTheGun > 0)
        {
            ...
            if (bullet)
            	...
            	holdFlash = Instantiate(muzzelFlash[randomNumberForMuzzleFlash], muzzelSpawn.transform.position /*- muzzelPosition*/, muzzelSpawn.transform.rotation * Quaternion.Euler(0,0,90) ) as GameObject;
            ...
            if (Physics.Raycast(bulletSpawnPlace.transform.position, bulletSpawnPlace.transform.forward, out hit))
            {
                ...
                {
                    // Pobierz materiał trafionego obiektu
                    MaterialType hitMaterial = objectHealth.materialType;

                    // Pobierz bieżący typ broni
                    WeaponType currentType = currentWeaponType;

                    // Sprawdź, czy bieżący materiał broni jest w tabeli obrażeń
                    if (damageTable.GetDamage(currentType, hitMaterial) != 0)
                    {
                        // Pobierz obrażenia dla bieżącego materiału broni i trafionego materiału
                        damage = damageTable.GetDamage(currentType, hitMaterial);
                    }
                    else
                    {
                        // Użyj domyślnych obrażeń broni
                        damage = 10;
                    }
